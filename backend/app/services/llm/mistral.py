...
class MistralProvider(LLMProvider):
    """Mistral AI API provider, called via raw HTTP (no SDK dependency)."""
    ...
    async def tag_article(
        ...
        try:
            ...
            logger.warning("mistral: tag_article timeout for model %s", self.model)
            return TaggingResult()
        except Exception as exc:
            logger.error("mistral: tag_article error: %s", exc)
            return TaggingResult()
    ...
    async def generate_digest(
        ...
        try:
            ...
            logger.error("mistral: generate_digest error: %s", exc)
            raise

    async def generate_text(self, prompt: str, max_tokens: int = 500) -> str:
        try:
            return await self._chat_complete(prompt, max_tokens=max_tokens)
        except Exception as exc:
            logger.error("mistral: generate_text error: %s", exc)
            return ""

    async def health_check(self) -> bool:
        try:
            async with httpx.AsyncClient(timeout=5.0) as client:
                res = await client.get(f"{self.base_url}/v1/models", headers=self._headers())
